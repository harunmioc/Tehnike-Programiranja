...
class Datum {
    int d, m, g;
public:
    friend istream& operator >> (istream& tok, Datum& dat);
...
public:
    ...
    friend istream& operator >> (istream& tok, Mjerenje& m); 
    friend ostream& operator << (ostream& out, const Mjerenje& m);
    
};

istream& operator >> (istream& tok, Datum& dat) {
    char c;
    tok >> dat.d >> c >> dat.m >> c >> dat.g;
    return tok;
}
